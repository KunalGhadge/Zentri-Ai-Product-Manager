  ...
  return (
    <nav className="fixed top-4 left-0 right-0 z-50 px-6 md:px-16 py-3 flex items-center justify-between pointer-events-none">
      <div className="flex items-center pointer-events-auto">
        <Link href="/" className="h-10 px-4 bg-white/5 rounded-lg flex items-center justify-center backdrop-blur-xl border border-white/10 overflow-hidden group hover:bg-white/10 transition-all">
          <img 
            src="/logo-icon.png" 
            alt="Zentri Logo" 
      ...
      <div className="hidden md:flex items-center liquid-glass rounded-full px-1.5 py-1 pointer-events-auto">
        {["Home", "Product", "Decisions", "Framework", "Pricing"].map((link) => (
          <Link
            key={link}
